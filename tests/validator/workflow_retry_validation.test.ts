...
describe('workflow retry validation', () => {
  ...
  const wf = validate(`page P { action act() {} }\nworkflow R { trigger { on create(Order) } retry { max: 3, backoff: fixed } steps { run act() } }`);
    expect((wf as any).retry.max).toBe(3);
  });
  test('rejects invalid max', () => {
  const src = `page P { action act() {} }\nworkflow R { trigger { on create(Order) } retry { max: -1, backoff: exponential } steps { run act() } }`;
    expect(() => validate(src)).toThrow(/retry.max/);
  });
  test('rejects unknown key', () => {
  const src = `page P { action act() {} }\nworkflow R { trigger { on create(Order) } retry { foo: 2 } steps { run act() } }`;
