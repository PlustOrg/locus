...
describe('Next app generation conditional on pages', () => {
  ...
  const { files } = buildOutputArtifacts({ database:{ entities:[] }, pages:[], components:[], designSystem:undefined, stores:[], workflows:[] } as any, { srcDir: '/proj' });
    expect(Object.keys(files).some(f=>f.startsWith('next-app/'))).toBe(false);
  });
  test('next-app files present when pages exist', () => {
  const { files } = buildOutputArtifacts({ database:{ entities:[] }, pages:[{ name:'Home', ui:'ui { <div/> }' }], components:[], designSystem:undefined, stores:[], workflows:[] } as any, { srcDir: '/proj' });
