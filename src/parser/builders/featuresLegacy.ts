...
import { parseStateBlockNode, parseStateDecls } from './stateHelpers';
...
// (state parsing moved to stateHelpers.ts)
...
export function buildFeatureBlocksLegacy(pageNodes: CstNode[], compNodes: CstNode[], storeNodes: CstNode[], originalSource: string) {
  ...
  if (stateBlocks.length) page.state = parseStateBlockNode(stateBlocks[0], originalSource);
  ...
  const stateBlocks = (ch['stateBlock'] as CstNode[]) || [];
  if (stateBlocks.length) store.state = parseStateBlockNode(stateBlocks[0], originalSource);
    if (!store.state) {
